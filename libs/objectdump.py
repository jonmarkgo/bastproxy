...
http://salmon-protocol.googlecode.com/svn-history/trunk/salmon-playground/dumper.py
"""
...
class Dumper:
    ...
    def dump_dict (self, dict, depth, indent, shallow_attrs=()):
        ...
        for k in keys:
            val = dict[k]
            if k in shallow_attrs:
                self._writeln("%s%s: %s" % (indent, k, shallow_dump (val)))
            elif short_value (val):
    ...
    def dump_instance (self, inst, depth, indent, summarize=1):
        ...
        tvars = vars(inst)
        self.dump_dict (tvars, depth, indent, shallow_attrs)
...
def shallow_dump(val):
    if atomic_type(type(val)) or is_instance(val) or is_class(val):
        return `val`

    else:
        return object_summary (val) + ': values will not be dumped'
        #return object_summary (val)
