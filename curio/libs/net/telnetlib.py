...
class Telnet(object):
  ...
  async def readdatafromsocket(self):
    """
    read 1024 bytes from the socket
    """
    buf = await self.sock.recv(1024)
    self.msg("recv %r" % buf)
    return buf
  ...
  async def handle_write(self):
    ...
    sent = await self.send(self.outbuffer)
  ...
  def handle_error(self):
    ...
    hand an error
    """
    print("Telnet error: %s" % self.ttype)
  ...
  async def fill_rawq(self):
    ...
    await buf = self.readdatafromsocket()
